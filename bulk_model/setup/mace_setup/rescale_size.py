...
def change_box_output_then_verify(lammps_exe, box, input_xyz, output_xyz, x_bounds, y_bounds, z_bounds,scale,ion,cation):
    '''
    Given a lammps system, changes the box size and verifies the density
    '''
    original_box = box
...
# Change the box
change_box all x final {x_bounds[0]} {x_bounds[1]} y final {y_bounds[0]} {y_bounds[1]} z final {z_bounds[0]} {z_bounds[1]}

# Output the modified structure to XYZ file
dump output_xyz all xyz 1 {output_xyz}
dump_modify output_xyz element O H {cation} N O
run 0
undump output_xyz
