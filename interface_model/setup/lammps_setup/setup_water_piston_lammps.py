'''
This script sets up a piston/sandwich system simulation for pure water
...
import setup_water_one_run_vacuum_lammps as setup_water_one_run_vacuum_lammps
import os
import numpy as np
...
n_h2o = 32 #2500
molecule_names = ['h2o']
n_molecules = np.array([n_h2o],dtype=int)
...
for replicate in replicates:
    run_name = f'{system_name}/water_rep{replicate}/'
    os.makedirs(os.path.join(current_dir,run_name), exist_ok=True)
    
    setup_water_one_run_vacuum_lammps.run_setup(run_name, n_molecules, molecule_names, 
