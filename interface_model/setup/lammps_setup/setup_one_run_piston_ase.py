"""
setup up one run for the piston simulation
...
def create_slab(slab_size):
    ...
    shift_slab = surface(shift_atoms, (1,1,1), layers=1, vacuum=0)
    ...
    slab = surface(bulk_tih2, (1,1,1), layers = 6,vacuum = shift_slab.cell[2][2]/2)
    slab = slab.repeat((slab_size,slab_size,1))
    positions = slab.get_positions()
    positions[:, 2] += shift_slab.cell[2][2]/2  # shift in z-direction
    ...
    bottom_slab = surface(bottom_atoms, (1,1,1), layers=1, vacuum=0)
    bottom_slab = bottom_slab.repeat((slab_size,slab_size,1))
    # positions = bottom_slab.get_positions()
    # positions[:, 2] -= shift_slab.cell[2][2]  # shift in z-direction
    # bottom_slab.set_positions(positions)
    
    slab = slab + bottom_slab
...
def run_packmol_elyte(Lx, Ly, Lz, molecule_names, n_molecules, cation_name):   
    ...
    cation_xyz = f"1\n\n\
     {cation_name}          0.00000        0.00000        0.00000\n"
    with open('cation.xyz','w') as f:
        f.write(cation_xyz)
    packmol_in = "seed -1\n\n\
    tolerance 2.0 \n\n\
    filetype xyz \n\n\
    output " + output_file + "\n\n"
    #changed this to reflect half of the water since it's split up by the interface
    for molecule_id in range(len(molecule_names)):
        packmol_in += "structure " + molecule_names[molecule_id] + ".xyz\n\
        number " + str(int(n_molecules[molecule_id]/2)) + "\n\
...
def run_combined_packmol(slab_size, elyte_height, slab_height, Lz, sigmaM):
    # write packmol file to combine electrodes and electrolyte
    elyte_height_top = Lz/2 + elyte_height/2 + slab_height/2 # add 1 sigma to each interface
    elyte_height_bot = Lz/2 - (elyte_height/2 + slab_height/2)
    with open("packmol_template.inp","rt") as fin:
            with open(f'packmol_sandwich.inp','wt') as fout:
                for line in fin:
                    fout.write(line.replace('{ELYTE_HEIGHT_TOP}',str(elyte_height_top)).replace(
                        '{ELYTE_HEIGHT_BOT}',str(elyte_height_bot)).replace(
                        '{ELECTRODE_HEIGHT}',str(Lz/2)).replace(
                        '{SLAB_SIZE}',str(slab_size/2)))

    os.system('packmol < packmol_sandwich.inp')
...
def run_moltemplate():
    os.system('moltemplate.sh -xyz packed_box_sandwich.xyz system.lt ')
...
def run_setup(run_name, n_molecules, molecule_names, 
    ...
    elyte_height = approx_box_size/2 * 1.25 # under-pack the box for equilibration

    # create_ion_xyz()
    run_packmol_elyte(Lx, Ly, elyte_height, molecule_names, n_molecules, cation_name)

    #gap_between_electrodes = 25 # Angstrom
    Lz = slab.cell[2][2] + elyte_height*2 #+ gap_between_electrodes # total z length of simulation box

    run_combined_packmol(slab.cell[0][0], elyte_height, slab.cell[2][2], Lz, sigmaM)

    molecule_names = molecule_names + molecule_names + ['metal'] #adding two because of the two electrolytes
    n_molecules = list((n_molecules/2).astype(int)) + list((n_molecules/2).astype(int)) + [slab.get_number_of_atoms()]
    ...
    # create_system_in(coul_cutoff, lj_cutoff, temp, timestep, n_steps_equil)
    # create_system_nvt_in(coul_cutoff, lj_cutoff, temp, timestep, n_steps)
    # run_moltemplate()
    ...
    atoms = read('packed_box_sandwich.xyz')
    atoms.set_cell([Lx,Ly,Lz])
    atoms.set_pbc([True, True, True])
    write('packed_box_sandwich.xyz', atoms)
    os.system(f'cp packed_box_sandwich.xyz {run_name}/init.xyz')