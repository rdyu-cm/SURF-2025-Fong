...
def run_combined_packmol(slab_size, elyte_height, slab_height, Lz, sigmaM):
    ...
    elyte_height_top = Lz/2 + elyte_height/2 + slab_height/2 # no - add 1 sigma to each interface
...
def create_system_nvt_in(cation,coul_cutoff, lj_cutoff, temp, timestep, n_nvt_steps):
    with open("system-template-nvt-mace.in","rt") as fin:
...
def run_setup(run_name, n_molecules, molecule_names, 
    ...
    elyte_height = approx_box_size/2 # under-pack the box for equilibration
    ...
    molecule_names = molecule_names + molecule_names + ['metal_Ti','metal_H'] #adding two because of the two electrolytes
    n_molecules = list((n_molecules/2).astype(int)) + list((n_molecules/2).astype(int)) + [int(slab.get_number_of_atoms()/3),int(2*slab.get_number_of_atoms()/3)]
