"""
setup up one run for the piston simulation (pure water)
...
def run_packmol_elyte(Lx, Ly, Lz, molecule_names, n_molecules, cation_name):   
    ...
    filetype xyz \n\n\
    output " + output_file + "\n\n"
    #reversed - changed this to reflect half of the water since it's split up by the interface
...
def run_combined_packmol(slab_size, elyte_height, slab_height, Lz, Lx, Ly, sigmaM):
    ...
    write('packed_box_tower.xyz', atoms)
    # with open('packed_box_tower.xyz','r') as fin:
    #     lines = fin.readlines()
    # add_vacuum = lines[1].split()
    # add_vacuum[8] = f'{str(float(add_vacuum[8].split("\"")) + 12-3.128)}\"'
    # lines[1] = ' '.join(add_vacuum)
    # with open('packed_box_tower.xyz','w') as fout:
...
def create_system_nvt_in(cation,coul_cutoff, lj_cutoff, temp, timestep, n_nvt_steps):
    with open("system-template-water-nvt-mace.in","rt") as fin:
